...
#################################################### 
{
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }: 
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in {
        packages.vongola = pkgs.rustPlatform.buildRustPackage {
          pname = "vongola";
          version = "0.1.0";
          src = ./.;
          cargoLock.lockFile = ./Cargo.lock;
          nativeBuildInputs = [ pkgs.pkgconf ]; # For linking
          buildInputs = [ pkgs.openssl pkgs.clang pkgs.cmake pkgs.openssl.dev ];
        };
        devShells.default = pkgs.mkShell {
          buildInputs = [
            rustToolchain
            pkgs.pkgconf pkgs.openssl pkgs.clang pkgs.cmake
            pkgs.git pkgs.curl pkgs.gcc pkgs.make
          ];
          shellHook = ''
