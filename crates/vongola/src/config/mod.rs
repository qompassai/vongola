...
impl Default for Path {
    fn default() -> Self {
        Self {
            lets_encrypt: PathBuf::from("/etc/vongola/letsencrypt"),
        }
    }
}
#[derive(Debug, Serialize, Deserialize, Clone)]
...
pub struct RouteSslCertificate {
    ...
    /// retrieved from the path or object storage (or generated from
...
pub struct Logging {
    /// If logging is enabled at all. Setting this to `false` will disable all
    /// logging output.
    #[arg(
        long = "log.enabled",
        required = false,
...
/// A configuration file (YAML, TOML or through ENV) will be parsed into this
/// struct. Example:
///
/// ```yaml
/// 
...
///   production: true
/// paths:
///   config_file: "/etc/vongola/config.toml"
...
pub(crate) struct Config {
    ...
    ///
    /// The configuration file should be named either `vongola.hcl` or
    /// `vongola.yaml`
    ///
...
impl Default for Config {
    fn default() -> Self {
        Config {
            config_path: Cow::Borrowed("/etc/vongola/config"),
            service_name: Cow::Borrowed("vongola"),
...
//     fn from<T: figment::Provider>(provider: T) -> Result<Config,
// figment::Error> {         Figment::from(provider).extract()
//     }
...
/// This allows the `Config` struct to be used as a configuration provider with
/// *defaults*.
impl Provider for Config {
    fn metadata(&self) -> figment::Metadata { figment::Metadata::named("vongola") }
...
/// In theory one could create all 3 configurations formats and they will
/// overlap each other
///
...
pub fn load(fallback: &str) -> Result<Config, figment::Error> {
    ...
    let config: Config = Figment::new()
        ...
        .merge(Yaml::file(format!("{path_with_fallback}/vongola.yml")))
        .merge(Yaml::file(format!("{path_with_fallback}/vongola.yaml")))
        .merge(Hcl::file(format!("{path_with_fallback}/vongola.hcl")))
        .merge(Env::prefixed("VONGOLA_").split("__"))
        .extract()?;
...
mod tests {
    ...
    fn test_load_config_from_yaml() {
        figment::Jail::expect_with(|jail| {
            ...
            assert_eq!(proxy_config.service_name, "vongola");
    ...
    fn test_load_config_from_yaml_and_env_vars() {
        figment::Jail::expect_with(|jail| {
            jail.create_file(
                format!("{}/vongola.yaml", jail.directory().to_str().unwrap()),
                helper_config_file(),
            )?;
            jail.set_env("VONGOLA_SERVICE_NAME", "new_name");
            jail.set_env("VONGOLA_LOGGING__ENABLED", "false");
            jail.set_env("VONGOLA_LOGGING__LEVEL", "warn");
            jail.set_env("VONGOLA_DOCKER__ENABLED", "true");
            jail.set_env("VONGOLA_DOCKER__INTERVAL_SECS", "30");
            jail.set_env("VONGOLA_DOCKER__ENDPOINT", "http://localhost:2375");
            jail.set_env("VONGOLA_LETS_ENCRYPT__STAGING", "false");
            jail.set_env("VONGOLA_LETS_ENCRYPT__EMAIL", "my-real-email@domain.com");
            jail.set_env("VONGOLA_LETS_ENCRYPT__RENEW_INTERVAL_SECS", "60");
            jail.set_env(
                "VONGOLA_ROUTES",
                r#"[{
    ...
    fn test_load_config_with_defaults_and_yaml() {
        figment::Jail::expect_with(|jail| {
            ...
            assert_eq!(path.key.as_os_str(), "/etc/vongola/certs/my-host.key");
            assert_eq!(path.pem.as_os_str(), "/etc/vongola/certs/my-host.pem");
